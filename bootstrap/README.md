...
2. Create artifacts/backlog.csv using the structure from bootstrap/backlog-template.csv with:
   ...
   - status values: 🔲 Backlog | ✅ Ready | 🔄 In Progress | 👀 In Review | Done | 🚫 Blocked | 📦 Deferred
   - copilot_suitable: Yes | Partial | No (required on every row — drives AI productivity KPI)
...
| **Status** | `🔲 Backlog`, `✅ Ready`, `🔄 In Progress`, `👀 In Review`, `Done`, `🚫 Blocked`, `📦 Deferred` | Gray → Green → Blue → Purple → Dark Green → Red → Light Blue |
